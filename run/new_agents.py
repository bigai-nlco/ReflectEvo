...
class BatchReactReflectAgent(ABC):
    ...
    def __post_init__(self):
        # TODO what encoding should be used here?
    ...
    def run(self, reset=True, reflection=None, setting=0, trail=0) -> None:#setting：1.不用scratchpad 2.用scratchpad
    ...
    def step(self) -> None:
        ...
        #     action_type, argument = 'Search', real_action
    ...
    def prompt_agent(self, prefix=None) -> str:
        #return format_step(self.reason_llm(self._build_agent_prompt(), prefix=prefix)) 不经过这一步了，不然代码问题没法搞
        # 这里不用前缀，发现并未变快。可能是串联的调用reason llm导致
...
class BatchCOTReflectAgent:
    ...
    def run_SFT(self, reflection=None,setting=0,trail=0,is_free_text=False) -> None:
        if reflection is None:
            ...
            self.reset()
            #print(f"this is scratchpad{self.scratchpad}")
        else:
            self.reflections = [reflection]
            self.reflections_str = format_reflections(self.reflections,self.scratchpad,setting=setting,trail=trail)
            #print(self.reflections_str)
    ...
    def step(self,is_free_text=False) -> None:
        ...
        if thought.find("[BEGIN]"):# 这个是针对mbpp的模板
        ...
        argument = ""
        cnt = 0
        while cnt < self.max_retry: # 这里不应该引入多次尝试输出动作，因为temp为0
            cnt += 1
            action = self.prompt_agent(prefix=f"Action: ")
            print("LLM Action输出：", action)
            # 这里把原始action的处理([]还是<<<>>>等)放到了环境的函数里
            try:
                action_type, argument = self.env.parse_action(action)
                if len(action_type) > 0:
                    break
                ...
                pass  # Parsing failed
        #print("处理后的action: ",action_type, argument)
        self.scratchpad += " " + action
        #print(self.scratchpad.split("\n")[-1])      # 打印action
    ...
    def _build_reflection_prompt(self,trail=0, previous_answer=None) -> str:
        prompt = self.reflect_prompt
        #print(f"build reflection prompt, trail:{trail},previous_answer:{previous_answer}")
        if 'answer' in prompt:
            if trail>=3:
            ...
            if trail>=3:
                #print(f"trail:{trail},modify scratchpad")
                scratchpad = f"{previous_answer}\n{self.scratchpad}"
            else:
                scratchpad = self.scratchpad
    ...
    def _build_agent_prompt(self) -> str:
        #print(f"agent_prompt:{self.agent_prompt.format(examples=self.examples,reflections=self.reflections_str,question=self.question,scratchpad=self.scratchpad,)}")
        return self.agent_prompt.format(
            examples=self.examples,
            reflections=self.reflections_str,
            question=self.question,
