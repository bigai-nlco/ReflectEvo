...
model = args.model
task = args.task
peft_model_id = "/scratch/nlp/lijiaqi/RFL_new/model/" + args.version + model + '_SFT/' + task + args.num_epochs
...
cuda_memory()
assert task == 'logiqa', "task名称不对"
assert model == 'Meta-Llama-3-8B-Instruct', "model名称不对"
...
eval_df = eval_df.sample(n=200, random_state=42)  # 随机抽取 eval_size 个样本作为评估集
