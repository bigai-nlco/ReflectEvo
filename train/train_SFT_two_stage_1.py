...
def parse_args():
    ...
    parse.add_argument('--ebs', type=int, help='batch size', default=30)
    parse.add_argument('--ss', type=str, help='save strategy', default="steps")
    parse.add_argument('--lr', type=float, help='learning_rate', default=0.00001)
...
df['output'] = df['second_reason'].apply(lambda x: x.split("Observation:")[0])
# 创建Hugging Face的数据集
...
# 对每一行的 'scratchpad' 进行分割并获取需要的部分
eval_df['output'] = eval_df['second_reason'].apply(lambda x: x.split("Observation:")[0])
